...
def warn_if_simplified_chinese(text):
    simplified_markers = ["这", "个", "为", "后", "里", "听", "说", "过", "对", "会", "与", "岁", "时", "亲"]
    hits = sorted({char for char in simplified_markers if char in text})
    if hits:
        print(f"⚠️ 這次輸出可能混入簡體字，建議人工複查：{''.join(hits[:12])}")
...
10. title、excerpt、text 一律使用繁體中文。不要輸出簡體中文、英文標題或中英混雜。
11. 若原始素材是簡體中文，請自然轉寫為台灣讀者看得懂的繁體中文。
12. 不要捏造字幕中沒有的重大情節；可以重組敘事，但要忠於原始材料。
13. 如果素材裡出現「不要說」「不在今天題目」「不要寫進去」等排除指令，必須遵守，不可納入故事。
14. text 是要直接交給語音朗讀的故事文本。它必須像散文、小篇短文、文藝作品。
15. text 裡禁止出現欄位標籤、章節標題、hashtags、項目符號、Markdown、包裝文案、FB/LINE 字樣。
16. text 裡不要寫「【故事】」「【故事裡的光】」「#咖啡時光廊」這類會被朗讀出來的雜訊。
17. 請嚴格輸出以下 JSON 格式，不要加任何 markdown 標記：
18. 【極度重要】在 JSON 的字串值中，絕對不可使用半形雙引號 `"`！若需引號請一律使用全形 `「` 和 `」`！
19. 【極度重要】所有換行都必須使用字串 `\\n`，絕對不可產生真實的換行符號！
{{
    "title": "動人的標題(不超過15個字)",
...
"""
    try:
        ...
        warn_if_simplified_chinese(
            " ".join([data.get("title", ""), data.get("excerpt", ""), data.get("text", "")])
        )
        return data
