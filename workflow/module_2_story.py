...
11. 如果素材裡出現「不要說」「不在今天題目」「不要寫進去」等排除指令，必須遵守，不可納入故事。
12. text 是要直接交給語音朗讀的故事文本。它必須像散文、小篇短文、文藝作品。
13. text 裡禁止出現欄位標籤、章節標題、hashtags、項目符號、Markdown、包裝文案、FB/LINE 字樣。
14. text 裡不要寫「【故事】」「【故事裡的光】」「#咖啡時光廊」這類會被朗讀出來的雜訊。
15. 請嚴格輸出以下 JSON 格式，不要加任何 markdown 標記：
16. 【極度重要】在 JSON 的字串值中，絕對不可使用半形雙引號 `"`！若需引號請一律使用全形 `「` 和 `」`！
17. 【極度重要】所有換行都必須使用字串 `\\n`，絕對不可產生真實的換行符號！
{{
    "title": "動人的標題(不超過15個字)",
